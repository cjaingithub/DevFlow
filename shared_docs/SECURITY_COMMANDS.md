...
DevFlow uses a dynamic security system that controls which shell commands the AI agent can execute. This prevents potentially dangerous operations while allowing legitimate development commands.
...
│     └── .devflow-allowlist file                         │
└─────────────────────────────────────────────────────────────┘
```

## Automatic Detection

When you start a task, DevFlow analyzes your project and automatically allows commands based on detected technologies:
...
The analyzer saves its results to `.devflow-security.json` in your project root:
...
For commands that aren't auto-detected, create a `.devflow-allowlist` file in your project root:
...
2. **Add to allowlist:**
   ...
   ```bash
   echo "your-command" >> .devflow-allowlist
   ```

3. **Force re-analysis** (if detection seems wrong):
   - Delete `.devflow-security.json`
   - Restart the task

### Allowlist Changes Not Taking Effect

The security profile cache updates automatically when:
- `.devflow-allowlist` is modified (mtime changes)
- `.devflow-security.json` is modified
