...
> Design document for converting devflow from Electron to a containerized web application.

## Executive Summary

This document outlines the architecture for a Docker-native version of devflow that:
...
| POST | `/api/projects/{id}/initialize` | Initialize devflow in project |
| GET | `/api/projects/{id}/version` | Check devflow version |
...
WORKDIR /app
COPY devflow/requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Install API dependencies
COPY devflow/api/requirements.txt ./api-requirements.txt
...
services:
  app:
    ...
    volumes:
      ...
      - devflow-claude:/home/claude/.claude
    environment:
  ...
  falkordb:
    ...
    container_name: devflow-falkordb
    volumes:
      - falkordb-data:/data
    networks:
      - devflow-net
  ...
  graphiti-mcp:
    ...
    container_name: devflow-graphiti
...
1. OAuth tokens stored in named volume (`devflow-claude`)
