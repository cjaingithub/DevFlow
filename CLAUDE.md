...
DevFlow is a multi-agent autonomous coding framework that builds software through coordinated AI agent sessions. It uses the Claude Agent SDK to run agents in isolated workspaces with security controls.
...
Each spec in `.devflow/specs/XXX-name/` contains:
...
DevFlow uses git worktrees for isolated builds. All branches stay LOCAL until user explicitly pushes:
...
**Key principles:**
- ONE branch per spec (`devflow/{spec-name}`)
...
**CRITICAL: When submitting PRs to AndyMik90/devflow, always target the `develop` branch, NOT `main`.**
...
5. Create PR targeting `develop`: `gh pr create --repo AndyMik90/devflow --base develop`
...
Security profile cached in `.devflow-security.json`.

### Claude Agent SDK Integration

**CRITICAL: DevFlow uses the Claude Agent SDK for ALL AI interactions. Never use the Anthropic API directly.**
...
DevFlow uses Graphiti as its primary memory system with embedded LadybugDB (no Docker required):
...
- Memory data stored in `.devflow/specs/XXX/graphiti/`
...
- `.devflow/specs/` - Per-project data (specs, plans, QA reports, memory) - gitignored
