...
This document covers terminal-only usage of DevFlow. **For most users, we recommend using the [Desktop UI](#) instead** - it provides a better experience with visual task management, progress tracking, and automatic Python environment setup.

## When to Use CLI

- You prefer terminal workflows
- You're running on a headless server
- You're integrating DevFlow into scripts or CI/CD
...
DevFlow uses Git worktrees for isolated builds:

```bash
# Test the feature in the isolated workspace
cd .worktrees/devflow/
