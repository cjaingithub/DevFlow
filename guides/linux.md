...
```bash
flatpak install --user apps/frontend/dist/devflow-*.flatpak
```

### Running from Flatpak

```bash
flatpak run com.devflow.devflow
```
...
```bash
# Make executable
chmod +x devflow-*-linux-x86_64.AppImage

# Run
./devflow-*-linux-x86_64.AppImage
```
...
```bash
sudo dpkg -i devflow-*-linux-amd64.deb
```
...
ldd ./devflow-*-linux-x86_64.AppImage

# Try running with debug output
./devflow-*-linux-x86_64.AppImage --verbose
```
