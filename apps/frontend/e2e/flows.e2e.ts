...
// Test data directory
const TEST_DATA_DIR = '/tmp/devflow-ui-e2e';
...
function setupTestEnvironment(): void {
  ...
  mkdirSync(path.join(TEST_PROJECT_DIR, 'devflow', 'specs'), { recursive: true });
}
...
function createTestSpec(specId: string, status: 'pending' | 'in_progress' | 'completed' = 'pending'): void {
  const specDir = path.join(TEST_PROJECT_DIR, 'devflow', 'specs', specId);
  ...
  test.skip('should open directory picker on add project click', async () => {
    ...
    await app.evaluate(({ dialog }) => {
      dialog.showOpenDialog = async () => ({
        canceled: false,
        filePaths: ['/tmp/devflow-ui-e2e/test-project']
      });
    });
...
test.describe('E2E Test Infrastructure', () => {
  ...
  test('should create test specs correctly', () => {
    ...
    const specDir = path.join(TEST_PROJECT_DIR, 'devflow', 'specs', '001-test-spec');
  ...
  test('should create specs with different statuses', () => {
    ...
    const specsDir = path.join(TEST_PROJECT_DIR, 'devflow', 'specs');
...
test.describe('E2E Flow Verification (Mock-based)', () => {
  test('Add Project flow should validate project path', async () => {
    ...
    // Check for devflow directory detection
    const autoBuildPath = path.join(projectPath, 'devflow');
