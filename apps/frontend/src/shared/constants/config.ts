...
// IMPORTANT: All paths use .devflow/ (the installed instance), NOT devflow/ (source code)
export const AUTO_BUILD_PATHS = {
  SPECS_DIR: '.devflow/specs',
  ROADMAP_DIR: '.devflow/roadmap',
  IDEATION_DIR: '.devflow/ideation',
  ...
  PROJECT_INDEX: '.devflow/project_index.json',
  GRAPHITI_STATE: '.graphiti_state.json'
} as const;

/**
 * Get the specs directory path.
 * All specs go to .devflow/specs/ (the project's data directory).
 */
export function getSpecsDir(autoBuildPath: string | undefined): string {
  const basePath = autoBuildPath || '.devflow';
