...
import { FlowchartImporter } from './components/flowchart-importer';
import { ReferenceGenerator } from './components/reference-generator';
...
export function App() {
  ...
  // GitHub setup state (shown after DevFlow init)
  ...
  // Check if selected project needs initialization (e.g., .devflow folder was deleted)
  useEffect(() => {
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        const errorMessage = result?.error || 'Failed to initialize DevFlow. Please try again.';
  ...
  return (
    <ViewStateProvider>
      ...
      <div className="flex h-screen bg-background">
          ...
          <main className="flex-1 overflow-hidden">
            {selectedProject ? (
              <>
                ...
                {activeView === 'flowchart-importer' && (activeProjectId || selectedProjectId) && (
                  <FlowchartImporter projectId={activeProjectId || selectedProjectId!} />
                )}
                {activeView === 'reference-generator' && (activeProjectId || selectedProjectId) && (
                  <ReferenceGenerator projectId={activeProjectId || selectedProjectId!} />
                )}
              </>
            ) : (
              <WelcomeScreen
        ...
        {/* GitHub Setup Modal - shows after DevFlow init to configure GitHub */}
        {gitHubSetupProject && (
          <GitHubSetupModal
