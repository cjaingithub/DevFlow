...
/**
 * Check devflow version status for a project
 */
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      if (result.data.success) {
        console.log('[ProjectStore] Updating project autoBuildPath to .devflow');
        store.updateProject(projectId, { autoBuildPath: '.devflow' });
      } else {
