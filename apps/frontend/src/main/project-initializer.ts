 ...
 */
const GITIGNORE_ENTRIES = ['.devflow/'];
...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (existsSync(gitignorePath)) {
    ...
    writeFileSync(gitignorePath, '# DevFlow data directory\n' + entriesToAdd.join('\n') + '\n');
  }
...
/**
 * Data directories created in .devflow for each project
 */
...
/**
 * Check if project is initialized (has .devflow directory)
 */
export function isInitialized(projectPath: string): boolean {
  const dotAutoBuildPath = path.join(projectPath, '.devflow');
  return existsSync(dotAutoBuildPath);
}

/**
 * Initialize devflow data directory in a project.
 *
 * Creates .devflow/ with data directories (specs, ideation, insights, roadmap).
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  // Check git status - DevFlow requires git for worktree-based builds
  const gitStatus = checkGitStatus(projectPath);
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    debug('Git check failed', { gitStatus });
    return {
      success: false,
      error: gitStatus.error || 'Git repository required. DevFlow uses git worktrees for isolated builds.'
    };
  }
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Already initialized - .devflow exists');
    return {
      success: false,
      error: 'Project already has devflow initialized (.devflow exists)'
    };
  }

  try {
    debug('Creating .devflow data directory', { dotAutoBuildPath });

    // Create the .devflow directory
    ...
    // Update .gitignore to exclude .devflow/
...
/**
 * Ensure all data directories exist in .devflow.
...
/**
 * Get the devflow folder path for a project.
 *
 * IMPORTANT: Only .devflow/ is considered a valid "installed" devflow.
 * The devflow/ folder (if it exists) is the SOURCE CODE being developed,
 * not an installation. This allows DevFlow to be used to develop itself.
 */
export function getAutoBuildPath(projectPath: string): string | null {
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Returning .devflow (installed version)');
    return '.devflow';
  }

  debug('No .devflow folder found - project not initialized');
  return null;
}
