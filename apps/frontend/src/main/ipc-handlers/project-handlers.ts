...
export function registerProjectHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<Project[]>> => {
      // Validate that .devflow folders still exist for all projects
      ...
      if (resetIds.length > 0) {
        console.warn('[IPC] PROJECT_LIST: Detected missing .devflow folders for', resetIds.length, 'project(s)');
      }
    ...
    async (_, projectId: string): Promise<IPCResult<InitializationResult>> => {
      try {
        ...
        if (result.success) {
          ...
          projectStore.updateAutoBuildPath(projectId, '.devflow');
        }
  ...
  // Version tracking for .devflow is removed since it only contains data
  ipcMain.handle(
      ...
      try {
        ...
        return {
          ...
          data: {
            ...
            updateAvailable: false // No updates for .devflow - it's just data
          }
        };
      } catch (error) {
        return {
  ...
  // Check if project has local devflow source (is dev project)
  ipcMain.handle(
