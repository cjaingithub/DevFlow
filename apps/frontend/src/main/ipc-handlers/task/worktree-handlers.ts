...
export function registerWorktreeHandlers(
   ...
   * Per-spec architecture: Each spec has its own worktree at .devflow/worktrees/tasks/{spec-name}/
   */
  ipcMain.handle(
    ...
    async (_, taskId: string): Promise<IPCResult<WorktreeStatus>> => {
      try {
        ...
        // Find worktree at .devflow/worktrees/tasks/{spec-name}/
      ...
      try {
        ...
        if (!pythonEnvManager.isEnvReady()) {
          ...
          if (autoBuildSource) {
            ...
            return { success: false, error: 'Python environment not ready and DevFlow source not found' };
          }
        }
        ...
        if (!sourcePath) {
          return { success: false, error: 'DevFlow source not found' };
        }

        const runScript = path.join(sourcePath, 'run.py');
        const specDir = path.join(project.path, project.autoBuildPath || '.devflow', 'specs', task.specId);
        ...
        return new Promise((resolve) => {
          ...
          const handleProcessExit = async (code: number | null, signal: string | null = null) => {
            ...
            if (code === 0) {
                ...
                if (isGitWorkTree(project.path)) {
                  try {
                    ...
                    if (!hasActualStagedChanges) {
                      ...
                      const specBranch = `devflow/${task.specId}`;
                      try {
              ...
              if (isStageOnly && !hasActualStagedChanges && mergeAlreadyCommitted) {
                ...
                try {
                  if (worktreePath && existsSync(worktreePath)) {
                    ...
                    const taskBranch = `devflow/${task.specId}`;
                    try {
              ...
              if (worktreePath) {
                const worktreeSpecDir = path.join(worktreePath, project.autoBuildPath || '.devflow', 'specs', task.specId);
      ...
      try {
        ...
        if (!pythonEnvManager.isEnvReady()) {
          ...
          if (autoBuildSource) {
            ...
            console.error('[IPC] DevFlow source not found');
            return { success: false, error: 'Python environment not ready and DevFlow source not found' };
          }
        }
