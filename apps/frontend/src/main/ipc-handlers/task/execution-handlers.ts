    ...
    (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      if (!gitStatus.isGitRepo) {
        ...
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          'Git repository required. Please run "git init" in your project directory. DevFlow uses git worktrees for isolated builds.'
        );
        return;
      }
    ...
    async (
      ...
      if (approved) {
        ...
        if (hasWorktree) {
          ...
          // IMPORTANT: Exclude .devflow directory to preserve specs and worktree data
          const cleanResult = spawnSync('git', ['clean', '-fd', '-e', '.devflow'], {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: 'pipe'
          });
          if (cleanResult.status === 0) {
            console.log('[TASK_REVIEW] Cleaned untracked files in main (excluding .devflow)');
          }
