...
/**
 * Auto-detect the devflow source path relative to the app location.
...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  for (const p of possiblePaths) {
    ...
    // This prevents matching legacy 'devflow/' directories that don't have the runners
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect DevFlow source path. Please configure manually in settings.');
...
export function registerSettingsHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<AppSettings>> => {
      ...
      // See: https://github.com/AndyMik90/devflow/issues/XXX
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# DevFlow Framework Environment Variables',
          '# Managed by DevFlow UI',
          '',
