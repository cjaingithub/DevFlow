...
function getGitHubDir(project: Project): string {
  return path.join(project.path, '.devflow', 'github');
}
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        ...
        try {
          ...
          if (options?.forceApprove) {
            ...
            body += `*Generated by DevFlow*`;
          } else {
            // Standard review format
            body = `## 🤖 DevFlow PR Review\n\n${result.summary}\n\n`;
            ...
            body += `---\n*This review was generated by DevFlow.*`;
          }
    ...
    async (_, projectId: string, prNumber: number, body: string): Promise<boolean> => {
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        try {
          ...
          const tmpFile = join(project.path, '.devflow', 'tmp_comment_body.txt');
          try {
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string, prNumber: number): Promise<NewCommitsCheck> => {
      ...
      const result = await withProjectOrNull(projectId, async (project) => {
        ...
        const githubDir = path.join(project.path, '.devflow', 'github');
