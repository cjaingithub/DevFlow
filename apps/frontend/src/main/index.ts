...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId('com.devflow.ui');
  ...
  // Load settings and configure agent manager with Python and devflow paths
  ...
  try {
    ...
    if (validAutoBuildPath) {
      ...
      if (!specRunnerExists) {
        ...
        // Old structure: /path/to/project/devflow
        // New structure: /path/to/project/apps/backend
        let migrated = false;
        if (validAutoBuildPath.endsWith('/devflow') || validAutoBuildPath.endsWith('\\devflow')) {
          const basePath = validAutoBuildPath.replace(/[/\\]devflow$/, '');
