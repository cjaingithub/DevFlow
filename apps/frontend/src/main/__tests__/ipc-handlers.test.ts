...
function setupTestProject(): void {
  ...
  mkdirSync(path.join(TEST_PROJECT_PATH, 'devflow', 'specs'), { recursive: true });
}
...
describe('IPC Handlers', { timeout: 15000 }, () => {
  ...
  describe('task:list handler', () => {
    ...
    it('should return tasks when specs exist', async () => {
      ...
      // Create .devflow directory first (before adding project so it gets detected)
      mkdirSync(path.join(TEST_PROJECT_PATH, '.devflow', 'specs'), { recursive: true });

      // Add a project - it will detect .devflow
      const addResult = await ipcMain.invokeHandler('project:add', {}, TEST_PROJECT_PATH);
      const projectId = (addResult as { data: { id: string } }).data.id;

      // Create a spec directory with implementation plan in .devflow/specs
      const specDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '001-test-feature');
  ...
  describe('Agent Manager event forwarding', () => {
    ...
    it('should forward exit events with status change on failure', async () => {
      ...
      const specDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', 'task-1');
      mkdirSync(specDir, { recursive: true });
      writeFileSync(
