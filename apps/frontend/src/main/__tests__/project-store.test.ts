...
describe('ProjectStore', () => {
  ...
  describe('addProject', () => {
    ...
    it('should detect devflow directory if present', async () => {
      // Create .devflow directory (the data directory, not source code)
      mkdirSync(path.join(TEST_PROJECT_PATH, '.devflow'), { recursive: true });
      ...
      expect(project.autoBuildPath).toBe('.devflow');
    });
  ...
  describe('getTasks', () => {
    ...
    it('should read tasks from filesystem correctly', async () => {
      // Create spec directory structure in .devflow (the data directory)
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '001-test-feature');
    ...
    it('should determine status as backlog when no subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '002-pending');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as ai_review when all subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '003-complete');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as human_review when QA report rejected', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '004-rejected');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as human_review when QA report approved', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '005-approved');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as done when plan status is explicitly done', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.devflow', 'specs', '006-done');
      mkdirSync(specsDir, { recursive: true });
