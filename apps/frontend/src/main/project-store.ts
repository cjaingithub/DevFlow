...
export class ProjectStore {
  ...
  addProject(projectPath: string, name?: string): Project {
    ...
    if (existing) {
      // Validate that .devflow folder still exists for existing project
      ...
      if (existing.autoBuildPath && !isInitialized(existing.path)) {
        console.warn(`[ProjectStore] .devflow folder was deleted for project "${existing.name}" - resetting autoBuildPath`);
    ...
    // Determine devflow path (supports both 'devflow' and '.devflow')
  ...
  /**
   * Validate all projects to ensure their .devflow folders still exist.
   ...
   *
   * @returns Array of project IDs that were reset due to missing .devflow folder
   */
  validateProjects(): string[] {
    ...
    for (const project of this.data.projects) {
      ...
      // Check if .devflow folder still exists
      if (!isInitialized(project.path)) {
        console.warn(`[ProjectStore] .devflow folder missing for project "${project.name}" at ${project.path}`);
    ...
    if (hasChanges) {
      this.save();
      console.warn(`[ProjectStore] Reset ${resetProjectIds.length} project(s) due to missing .devflow folder`);
    }
