 ...
 *
 * Handles loading and saving profiles.json from the devflow directory.
...
/**
 * Get the path to profiles.json in the devflow directory
 */
export function getProfilesFilePath(): string {
  const userDataPath = app.getPath('userData');
  return path.join(userDataPath, 'devflow', 'profiles.json');
}
 ...
 * Creates the devflow directory if it doesn't exist
