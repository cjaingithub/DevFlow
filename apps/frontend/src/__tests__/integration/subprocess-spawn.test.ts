...
// devflow source path (for getAutoBuildSourcePath to find)
const AUTO_CLAUDE_SOURCE = path.join(TEST_DIR, 'devflow-source');

// Setup test directories
function setupTestDirs(): void {
  mkdirSync(TEST_PROJECT_PATH, { recursive: true });

  // Create devflow source directory that getAutoBuildSourcePath looks for
    ...
    it('should spawn Python process for spec creation', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: AUTO_CLAUDE_SOURCE,  // Process runs from devflow source directory
