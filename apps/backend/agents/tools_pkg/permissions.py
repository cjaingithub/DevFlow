...
def get_allowed_tools(
    ...
    # Add devflow tools ONLY if the MCP server is available
    # This prevents allowing tools that won't work because the server isn't running
    if "devflow" in required_servers and is_tools_available():
...
def _get_mcp_tools_for_servers(servers: list[str]) -> list[str]:
    ...
    for server in servers:
        ...
        # devflow tools are already added via config["auto_claude_tools"]
