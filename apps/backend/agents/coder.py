...
async def run_autonomous_agent(
    ...
    Args:
        ...
        spec_dir: Directory containing the spec (devflow/specs/001-name/)
    ...
    while True:
        ...
        if pause_file.exists():
            ...
            print("\nThen run again:")
            print(f"  python devflow/run.py --spec {spec_dir.name}")
            return
    ...
    if completed < total:
        content = [
            ...
            f"Run again: {highlight(f'python devflow/run.py --spec {spec_dir.name}')}",
        ]
    else:
        content = [
