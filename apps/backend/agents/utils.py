...
def sync_spec_to_source(spec_dir: Path, source_spec_dir: Path | None) -> bool:
    ...
    IMPORTANT: Since .devflow/ is gitignored, this sync happens to the
    local filesystem regardless of what branch the user is on. The worktree
    may be on a different branch (e.g., devflow/093-task), but the sync
    target is always the main project's .devflow/specs/ directory.
