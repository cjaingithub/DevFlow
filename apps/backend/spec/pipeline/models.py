...
def get_specs_dir(project_dir: Path) -> Path:
    ...
    IMPORTANT: Only .devflow/ is considered an "installed" devflow.
    The devflow/ folder (if it exists) is SOURCE CODE being developed,
    not an installation. This allows DevFlow to be used to develop itself.

    This function also ensures .devflow is added to .gitignore on first use.

    Args:
        project_dir: The project root directory

    Returns:
        Path to the specs directory within .devflow/
    """
    # Initialize .devflow directory and ensure it's in .gitignore
    init_auto_claude_dir(project_dir)

    # Return the specs directory path
    return project_dir / ".devflow" / "specs"
