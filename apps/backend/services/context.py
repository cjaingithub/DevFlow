...
Usage:
    ...
    python devflow/service_context.py --service backend --output backend/SERVICE_CONTEXT.md

    # Generate for all services (using project index)
    python devflow/service_context.py --all

    # Generate with custom project index
    python devflow/service_context.py --service frontend --index devflow/project_index.json
"""
...
class ServiceContextGenerator:
    ...
    def _load_project_index(self) -> dict:
        """Load project index from file (.devflow is the installed instance)."""
        index_file = self.project_dir / ".devflow" / "project_index.json"
