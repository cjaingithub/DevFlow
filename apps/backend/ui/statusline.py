...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/devflow/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .devflow or .devflow-status."""
    cwd = Path.cwd()

    # Check current directory - prioritize .devflow (installed instance)
    if (cwd / ".devflow").exists():
        return cwd
    if (cwd / ".devflow-status").exists():
        return cwd
    ...
    for parent in cwd.parents:
        if (parent / ".devflow").exists():
            return parent
        if (parent / ".devflow-status").exists():
            return parent
