...
def get_relative_spec_path(spec_dir: Path, project_dir: Path) -> str:
    ...
    Returns:
        Relative path string (e.g., "./devflow/specs/003-new-spec")
    """
    try:
        ...
        return f"./devflow/specs/{spec_dir.name}"
...
5. **Commit your changes:**
   ```bash
   git add .
   git commit -m "devflow: {subtask_id} - {description[:50]}"
   ```
...
def generate_planner_prompt(spec_dir: Path, project_dir: Path | None = None) -> str:
    ...
    if project_dir is None:
        # Infer: spec_dir is typically project/devflow/specs/XXX
