"""
DevFlow project initialization utilities.

Handles first-time setup of .devflow directory and ensures proper gitignore configuration.
"""

from pathlib import Path

# All entries that should be added to .gitignore for devflow projects
AUTO_CLAUDE_GITIGNORE_ENTRIES = [
    ".devflow/",
    ".devflow-security.json",
    ".devflow-status",
...
def ensure_gitignore_entry(project_dir: Path, entry: str = ".devflow/") -> bool:
    """
    ...
    Args:
        ...
        entry: The gitignore entry to add (default: ".devflow/")
    ...
    if gitignore_path.exists():
        ...
        content = "# DevFlow data directory\n"
        content += entry + "\n"
...
def ensure_all_gitignore_entries(project_dir: Path) -> list[str]:
    """
    Ensure all devflow related entries exist in the project's .gitignore file.
...
def init_auto_claude_dir(project_dir: Path) -> tuple[Path, bool]:
    """
    Initialize the .devflow directory for a project.

    Creates the directory if needed and ensures all devflow files are in .gitignore.
    ...
    auto_claude_dir = project_dir / ".devflow"

    # Create the directory if it doesn't exist
    dir_created = not auto_claude_dir.exists()
    auto_claude_dir.mkdir(parents=True, exist_ok=True)

    # Ensure all devflow entries are in .gitignore (only on first creation)
...
def get_auto_claude_dir(project_dir: Path, ensure_exists: bool = True) -> Path:
    """
    Get the .devflow directory path, optionally ensuring it exists.
    ...
    Returns:
        Path to the .devflow directory
    """
    if ensure_exists:
        auto_claude_dir, _ = init_auto_claude_dir(project_dir)
        return auto_claude_dir

    return Path(project_dir) / ".devflow"


def repair_gitignore(project_dir: Path) -> list[str]:
    """
    Repair an existing project's .gitignore to include all devflow entries.
