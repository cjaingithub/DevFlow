...
- Worktree path: .devflow/worktrees/tasks/{spec-name}/
- Branch name: devflow/{spec-name}
...
class WorktreeManager:
    ...
    Each spec gets its own worktree in .devflow/worktrees/tasks/{spec-name}/ with
    a corresponding branch devflow/{spec-name}.
    """

    def __init__(self, project_dir: Path, base_branch: str | None = None):
        self.project_dir = project_dir
        self.base_branch = base_branch or self._detect_base_branch()
        self.worktrees_dir = project_dir / ".devflow" / "worktrees" / "tasks"
    ...
    def _unstage_gitignored_files(self) -> None:
        ...
        plus any files in the .devflow directory which should never be merged.

        This is needed after a --no-commit merge because files that exist in the
        source branch (like spec files in .devflow/specs/) get staged even if
        ...
        # Files to unstage: gitignored files + .devflow directory files
        ...
        # 2. Always unstage .devflow directory files - these are project-specific
        # and should never be merged from the worktree branch
        auto_claude_patterns = [".devflow/", "devflow/specs/"]
        ...
        if files_to_unstage:
            print(
                f"Unstaging {len(files_to_unstage)} devflow/gitignored file(s)..."
            )
            # Unstage each file
    ...
    def get_worktree_path(self, spec_name: str) -> Path:
        ...
        # Legacy fallback (.worktrees/ instead of .devflow/worktrees/tasks/)
    ...
    def _check_branch_namespace_conflict(self) -> str | None:
        """
        Check if a branch named 'devflow' exists, which would block creating
        branches in the 'devflow/*' namespace.

        Git stores branch refs as files under .git/refs/heads/, so a branch named
        'devflow' creates a file that prevents creating the 'devflow/'
        directory needed for 'devflow/{spec-name}' branches.

        Returns:
            The conflicting branch name if found, None otherwise.
        """
        result = self._run_git(["rev-parse", "--verify", "devflow"])
        if result.returncode == 0:
            return "devflow"
        return None
    ...
    def create_worktree(self, spec_name: str) -> WorktreeInfo:
        ...
        # Check for branch namespace conflict (e.g., 'devflow' blocking 'devflow/*')
        ...
        if conflicting_branch:
            raise WorktreeError(
                ...
                f"\n"
                f"Git branch names work like file paths - a branch named 'devflow' prevents\n"
                f"creating branches under 'devflow/' (like 'devflow/{spec_name}').\n"
                f"\n"
    ...
    def merge_worktree(
        ...
        if no_commit:
            ...
            merge_args.extend(["-m", f"devflow: Merge {info.branch}"])
    ...
    def list_all_spec_branches(self) -> list[str]:
        """List all devflow branches (even if worktree removed)."""
        result = self._run_git(["branch", "--list", "devflow/*"])
        if result.returncode != 0:
            return []
