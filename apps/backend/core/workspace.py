...
gets its own isolated worktree in .devflow/worktrees/tasks/{spec-name}/.
...
def merge_existing_build(
    ...
    Called when user runs: python devflow/run.py --spec X --merge
    ...
    if not worktree_path:
        ...
        print(highlight(f"  python devflow/run.py --spec {spec_name}"))
        return False
    ...
    spec_branch = f"devflow/{spec_name}"
    ...
    if current_branch == spec_branch:
        ...
        print(highlight(f"  python devflow/run.py --spec {spec_name} --merge"))
        return False
        ...
        if smart_result is not None:
            ...
            if smart_result.get("success"):
                ...
                if had_conflicts or files_merged or ai_assisted:
                    ...
                    # User can delete with: python devflow/run.py --spec <name> --discard
    ...
    if success_result:
        ...
        if no_commit:
            ...
            print(muted(f"  python devflow/run.py --spec {spec_name} --discard"))
        else:
    ...
    try:
        ...
        if merge_tree_result.returncode != 0:
            ...
            for line in output.split("\n"):
                if "CONFLICT" in line:
                    ...
                    if match:
                        ...
                        # Skip .devflow files - they should never be merged
                        if (
                            file_path
            ...
            if not result["conflicting_files"]:
                ...
                # Filter out .devflow files - they should never be merged
...
def _resolve_git_conflicts_with_ai(
    ...
    spec_branch = git_conflicts.get("spec_branch", f"devflow/{spec_name}")
