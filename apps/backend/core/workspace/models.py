...
class MergeLock:
    ...
    Uses a lock file in .devflow/ to ensure only one merge operation
    ...
    def __init__(self, project_dir: Path, spec_name: str):
        ...
        self.lock_dir = project_dir / ".devflow" / ".locks"
...
class SpecNumberLock:
    ...
    def get_next_spec_number(self) -> int:
        ...
        main_specs_dir = self.project_dir / ".devflow" / "specs"
        max_number = max(max_number, self._scan_specs_dir(main_specs_dir))

        # 2. Scan all worktree specs
        worktrees_dir = self.project_dir / ".devflow" / "worktrees" / "tasks"
        if worktrees_dir.exists():
            for worktree in worktrees_dir.iterdir():
                if worktree.is_dir():
                    worktree_specs = worktree / ".devflow" / "specs"
