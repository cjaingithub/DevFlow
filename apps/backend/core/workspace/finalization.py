    ...
    if choice == WorkspaceChoice.TEST:
        ...
        if staging_path:
            ...
            if worktree_path:
                ...
                print(
                    highlight(
                        f"  cd {project_dir}/.devflow/worktrees/tasks/{spec_name}"
                    )
                )
        ...
        print(muted(f"  python devflow/run.py --spec {spec_name} --review"))
        print()
...
def review_existing_build(project_dir: Path, spec_name: str) -> bool:
    ...
    Called when user runs: python devflow/run.py --spec X --review
...
def discard_existing_build(project_dir: Path, spec_name: str) -> bool:
    ...
    Called when user runs: python devflow/run.py --spec X --discard
