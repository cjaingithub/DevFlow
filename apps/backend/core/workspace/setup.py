...
def choose_workspace(
    ...
    """
    Let user choose where devflow should work.
...
def copy_spec_to_worktree(
    ...
    # Use .devflow/specs/{spec_name}/ as the standard location
    # Note: devflow/ is source code, .devflow/ is the installed instance
    target_spec_dir = worktree_path / ".devflow" / "specs" / spec_name
...
def setup_workspace(
    ...
    # Ensure .devflow/ is in the worktree's .gitignore
    # This is critical because the worktree inherits .gitignore from the base branch,
    # which may not have .devflow/ if that change wasn't committed/pushed.
    # Without this, spec files would be committed to the worktree's branch.
    from init import ensure_gitignore_entry

    if ensure_gitignore_entry(worktree_info.path, ".devflow/"):
        debug(MODULE, "Added .devflow/ to worktree's .gitignore")
