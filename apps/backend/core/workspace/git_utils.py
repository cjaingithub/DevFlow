...
def get_existing_build_worktree(project_dir: Path, spec_name: str) -> Path | None:
    ...
    """
    # New path first
    new_path = project_dir / ".devflow" / "worktrees" / "tasks" / spec_name
    if new_path.exists():
        return new_path
...
def get_changed_files_from_branch(
    ...
    Args:
        ...
        exclude_auto_claude: If True, exclude .devflow directory files (default True)
    ...
    if result.returncode == 0:
        for line in result.stdout.strip().split("\n"):
            if line:
                ...
                if len(parts) == 2:
                    file_path = parts[1]
                    # Exclude .devflow directory files from merge
...
def _is_auto_claude_file(file_path: str) -> bool:
    """Check if a file is in the .devflow or devflow/specs directory.
