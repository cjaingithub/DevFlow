...
def load_project_mcp_config(project_dir: Path) -> dict:
    """
    Load MCP configuration from project's .devflow/.env file.
    ...
    """
    env_path = project_dir / ".devflow" / ".env"
...
def create_client(
    ...
    # Check if custom devflow tools are available
    ...
    # Load per-project MCP configuration from .devflow/.env
    ...
    worktree_markers = [
        "/.devflow/worktrees/tasks/",  # Spec/task worktrees
        "/.devflow/github/pr/worktrees/",  # PR review worktrees
    ...
    security_settings = {
        ...
        "permissions": {
            ...
            "allow": [
                ...
                # Allow original project's .devflow/ and .worktrees/ directories
    ...
    if "devflow" in required_servers and auto_claude_tools_enabled:
        mcp_servers_list.append(f"devflow ({agent_type} tools)")
    if mcp_servers_list:
