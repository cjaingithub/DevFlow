...
if sys.version_info < (3, 10):  # noqa: UP036
    sys.exit(
        f"Error: DevFlow requires Python 3.10 or higher.\n"
...
# Add devflow to path (parent of runners/)
...
dev_env_file = Path(__file__).parent.parent.parent / "dev" / "devflow" / ".env"
if env_file.exists():
...
Examples:
  ...
  python spec_runner.py --interactive
    ...
    # Find project root (look for devflow folder)
    project_dir = args.project_dir

    # Auto-detect if running from within devflow directory (the source code)
    if project_dir.name == "devflow" and (project_dir / "run.py").exists():
        # Running from within devflow/ source directory, go up 1 level
        project_dir = project_dir.parent
    elif not (project_dir / ".devflow").exists():
        # No .devflow folder found - try to find project root
        # First check for .devflow (installed instance)
        for parent in project_dir.parents:
            if (parent / ".devflow").exists():
                project_dir = parent
                break
    ...
    try:
        ...
        if not args.no_build:
            ...
            if not review_state.is_approved():
                ...
                print(
                    f"  {highlight(f'python devflow/review.py --spec-dir {orchestrator.spec_dir}')}"
                )
                print()
                print(
                ...
                example_cmd = (
                    'python devflow/spec_runner.py --task "..." --auto-approve'
                )
        ...
        print(
            f"To continue: python devflow/spec_runner.py --continue {orchestrator.spec_dir.name}"
        )
        sys.exit(1)
