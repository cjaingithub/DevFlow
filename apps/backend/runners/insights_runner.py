...
def load_project_context(project_dir: str) -> str:
    ...
    # Load project index if available (from .devflow - the installed instance)
    index_path = Path(project_dir) / ".devflow" / "project_index.json"
    ...
    roadmap_path = Path(project_dir) / ".devflow" / "roadmap" / "roadmap.json"
    ...
    # Load existing tasks
    tasks_path = Path(project_dir) / ".devflow" / "specs"
