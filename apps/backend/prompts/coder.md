...
- **Spec Location**: Where your spec files live (usually `./devflow/specs/{spec-name}/`)
...
SPEC_DIR="./devflow/specs/YOUR-SPEC-NAME"  # Replace with actual path from step 2
...
4. **Re-stage and retry** - `git add . ':!.devflow' && git commit ...`
...
# Add all files EXCEPT .devflow directory (spec files should never be committed)
git add . ':!.devflow'
...
git commit -m "devflow: Complete [subtask-id] - [subtask description]
...
**CRITICAL**: The `:!.devflow` pathspec exclusion ensures spec files are NEVER committed.
...
**Note:** The `build-progress.txt` file is in `.devflow/specs/` which is gitignored.
