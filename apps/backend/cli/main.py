...
Command-line interface for the DevFlow autonomous coding framework.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="DevFlow Framework - Autonomous multi-session coding agent",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # List all specs
  python devflow/run.py --list

  # Run a specific spec (by number or full name)
  python devflow/run.py --spec 001
  python devflow/run.py --spec 001-initial-app

  # Workspace management (after build completes)
  python devflow/run.py --spec 001 --merge     # Add build to your project
  python devflow/run.py --spec 001 --review    # See what was built
  python devflow/run.py --spec 001 --discard   # Delete build (with confirmation)

  # Advanced options
  python devflow/run.py --spec 001 --direct       # Skip workspace isolation
  python devflow/run.py --spec 001 --isolated     # Force workspace isolation

  # Status checks
  python devflow/run.py --spec 001 --review-status  # Check human review status
  python devflow/run.py --spec 001 --qa-status      # Check QA validation status
...
def main() -> None:
    ...
    if not args.spec:
        ...
        print("\nUsage:")
        print("  python devflow/run.py --list           # See all specs")
        print("  python devflow/run.py --spec 001       # Run a spec")
