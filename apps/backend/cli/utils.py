...
Shared utility functions for the DevFlow CLI.
"""
...
def setup_environment() -> Path:
    ...
    """
    # Add devflow directory to path for imports
    script_dir = Path(__file__).parent.parent.resolve()
    sys.path.insert(0, str(script_dir))

    # Load .env file - check both devflow/ and dev/devflow/ locations
    env_file = script_dir / ".env"
    dev_env_file = script_dir.parent / "dev" / "devflow" / ".env"
    if env_file.exists():
...
def find_spec(project_dir: Path, spec_identifier: str) -> Path | None:
    ...
    worktree_base = project_dir / ".devflow" / "worktrees" / "tasks"
    if worktree_base.exists():
        # Try exact match in worktree
        worktree_spec = (
            worktree_base / spec_identifier / ".devflow" / "specs" / spec_identifier
        )
        ...
        for worktree_dir in worktree_base.iterdir():
            if worktree_dir.is_dir() and worktree_dir.name.startswith(
                ...
                spec_in_worktree = (
                    worktree_dir / ".devflow" / "specs" / worktree_dir.name
                )
                if (
...
def validate_environment(spec_dir: Path) -> bool:
    ...
    if not get_auth_token():
        ...
        print("\nDevFlow requires Claude Code OAuth authentication.")
    ...
    if is_linear_enabled():
        ...
        )  # devflow/specs/001-name -> project root
