        ...
        if force_bypass_approval:
            ...
            content.extend(
                [
                    ...
                    f"  python devflow/review.py --spec-dir {spec_dir}",
                    "",
        ...
        if not skip_qa and should_run_qa(spec_dir):
            ...
            try:
                ...
                if qa_approved:
                    ...
                    print(
                        f"\nResume QA: python devflow/run.py --spec {spec_dir.name} --qa\n"
                    )
                ...
                print(f"Resume: python devflow/run.py --spec {spec_dir.name} --qa")
                qa_approved = False
...
def _handle_build_interrupt(
    ...
    content = [
        ...
        "",
        f"Run: {highlight(f'python devflow/run.py --spec {spec_dir.name}')}",
    ]
    if worktree_manager:
        content.append("")
